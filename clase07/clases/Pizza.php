...
class Pizza 
{
    ...
    public static function CrearVenta($p1, $mail)
    {
        ...
        foreach ($array as $pizza) {
            ...
            if($pizza->Equals($p1) && $pizza->cantidad > $p1->cantidad)
            {
                ...
                $consulta =$objetoAccesoDato->RetornarConsulta('INSERT INTO pizza (sabor, tipo, fecha, pedido) VALUES (:sabor,:tipo,:fecha,:pedido)');
    ...
    {
        $objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso();
        $consulta =$objetoAccesoDato->RetornarConsulta('SELECT SUM(cantidad) AS cantidadTotal FROM pizza');
        $consulta->execute(); 
